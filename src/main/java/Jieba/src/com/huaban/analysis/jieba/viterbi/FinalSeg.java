...
public class FinalSeg {
    ...
    public void viterbi(String sentence, List<String> tokens) {
        ...
        for (char state : states) {
            // compute the first char for each state.
            Double emP = emit.get(state).get(sentence.charAt(0));
            if (null == emP)
                emP = MIN_FLOAT;
            // compute the p(state)p(first char labeled as the state)
        ...
        for (int i = 1; i < sentence.length(); ++i) {
            ...
            for (char y : states) {
                ...
                for (char y0 : prevStatus.get(y)) {
                    ...
                    if (null == tranp)
                        tranp = MIN_FLOAT;
                    // junius, the possibility of transform from y0 to y.
                    // also consider the char labeled as state via variable emp.
                    tranp += (emp + v.get(i - 1).get(y0));

                    //store most possible transfer candidate.
                    if (null == candidate)
        ...
        // just consider whole sentence labeled completely.
